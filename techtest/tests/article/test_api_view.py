...
class ArticleTestCase(TestCase):
    ...
    def test_get_article(self):
        ...
        self.assertEqual(resp.json(), {'id': 1, 'authors':
            [{'id': 1, 'first_name': 'test', 'last_name': 'user'}],
                                       'title': 'test article',
                                       'regions': [
                                           {'id': 1, 'name': 'United Kingdom', 'code': 'UK'}],
                                       'content': 'test content'})

    def test_post_article_without_author_region(self):
        post_data = {"title": "fake title", "content": "fake content"}
        resp = self.client.post(f'/api/v1/articles/', data=json.dumps(post_data),
    ...
    def test_post_article_with_author(self):
        ...
        resp = self.client.post('/api/v1/articles/', data=json.dumps(post_data),
                                content_type="application/json")
        self.assertEqual(resp.status_code, 201)
        article = Article.objects.get(title="fake title")
        self.assertEqual(resp.status_code, 201)
    ...
    def test_put_article(self):
        ...
        post_data = {"title": "fake article", "content": "fake content",
                     "regions": [{"id": region.id}], "authors": [{"id": author.id}]}
        resp = self.client.put(f'/api/v1/article/{article.id}/', data=json.dumps(post_data),
                               content_type="application/json")
